...
def build_exp11_model_with_mask():
    ...
    attn_mask_input_1 = tf.keras.Input(shape=(6, 6), name='attention_mask_input_1', dtype=tf.bool)
    attn_mask_input_2 = tf.keras.Input(shape=(6, 6), name='attention_mask_input_2', dtype=tf.bool)
    ...
    def tile_mask(x):
        batch = tf.shape(x)[0]
        mask_expanded = tf.expand_dims(x, 1) # (batch, 1, 6, 6)
        mask_tiled = tf.tile(mask_expanded, [1, 250, 1, 1]) # (batch, 250, 6, 6)
        return tf.reshape(mask_tiled, [batch * 250, 6, 6])
    
    broadcasted_mask_1 = tf.keras.layers.Lambda(tile_mask, output_shape=(None, 6, 6))(attn_mask_input_1)
    broadcasted_mask_2 = tf.keras.layers.Lambda(tile_mask, output_shape=(None, 6, 6))(attn_mask_input_2)
    
    transformer_block_1 = TransformerBlockXAI(embed_dim=64, num_heads=4, ff_dim=128, name='cross_sector_transformer_1')
    cross_sector_att_1 = transformer_block_1(sector_cls, attention_mask=broadcasted_mask_1)
    
    transformer_block_2 = TransformerBlockXAI(embed_dim=64, num_heads=4, ff_dim=128, name='cross_sector_transformer_2')
    cross_sector_att_2 = transformer_block_2(cross_sector_att_1, attention_mask=broadcasted_mask_2)
    
    def extract_cls(x):
        return x[:, 0, :]
    phase_features_flat = tf.keras.layers.Lambda(extract_cls)(cross_sector_att_2)
    ...
    model = tf.keras.Model(inputs=[local_input, attn_mask_input_1, attn_mask_input_2], outputs=outputs)
...
def main():
    ...
    # We must tile the mask across the batch dimension
    train_mask_batch = np.repeat(full_mask[np.newaxis, :, :], len(X_train), axis=0)
    val_mask_batch = np.repeat(full_mask[np.newaxis, :, :], len(X_val), axis=0)
    
    print("\nTraining Two-Stage Cross-Sector Model (Exp 11A)...")
    tf.keras.backend.clear_session()
    
    # We will use Seed 42 for stability testing, or 300 since it worked last time. 
    # Let's use 42 to see if it trains.
    ...
    model.fit(
        [X_train, train_mask_batch, train_mask_batch], y_train,
        validation_data=([X_val, val_mask_batch, val_mask_batch], y_val),
        epochs=50,
        batch_size=32,
    ...
    val_loss, val_acc = model.evaluate([X_val, val_mask_batch, val_mask_batch], y_val, verbose=0)
    ...
    intact_preds = model.predict([X_val, val_mask_batch, val_mask_batch], verbose=0)
    ...
    print("\\nEvaluating Ablated Model (NO CROSS-SECTOR ATTENTION IN EITHER BLOCK)...")
    # Ablated mask: 
    ...
    ab_loss, ab_acc = model.evaluate([X_val, ablated_mask_batch, ablated_mask_batch], y_val, verbose=0)
    print(f"Ablated Both Blocks Validation Accuracy: {ab_acc:.4f}")
    
    ab_preds = model.predict([X_val, ablated_mask_batch, ablated_mask_batch], verbose=0)
    ab_planet_recall = np.mean(np.argmax(ab_preds[planet_indices], axis=1) == 1)
    print(f"Ablated Both Blocks Planet Recall: {ab_planet_recall:.4f}")

    print("\\nEvaluating Block 1 Ablated, Block 2 Intact...")
    b1_loss, b1_acc = model.evaluate([X_val, ablated_mask_batch, val_mask_batch], y_val, verbose=0)
    print(f"Block 1 Ablated Validation Accuracy: {b1_acc:.4f}")
    b1_preds = model.predict([X_val, ablated_mask_batch, val_mask_batch], verbose=0)
    b1_planet_recall = np.mean(np.argmax(b1_preds[planet_indices], axis=1) == 1)
    print(f"Block 1 Ablated Planet Recall: {b1_planet_recall:.4f}")
    # End of evaluations

...
def build_exp9b_model():
    """
    Exp 9B Architecture (Recurrent Sector Fusion)
    
    Local Branch:
    - Input: (5, 2000, 1) independent phase-folded sectors.
    - Encoder: Shared CNN applied to each sector.
    - Aggregation: LSTM (Sequence Modeling) replacing Mean Pooling / Flatten.
    ...
    shared_cnn = tf.keras.Sequential([
        ...
        tf.keras.layers.MaxPooling1D(pool_size=2),
        tf.keras.layers.GlobalAveragePooling1D(), # (batch, 64)
        tf.keras.layers.Dense(128, activation='relu')
    ], name='shared_sector_cnn')
    
    encoded_sectors = tf.keras.layers.TimeDistributed(shared_cnn)(local_input) # (batch, 5, 128)
    
    # RECURRENT SEQUENCE MODELING (Replaces Flatten from Exp 9A)
    # We use LSTM to learn temporal relationships across the 5 sectors
...
def main():
    print(f"\n==================================================")
    print(f"TRAINING EXP 9B: Recurrent Sector Fusion (LSTM)")
    print(f"==================================================")
    ...
    save_path = 'data/models/exp9b_model.keras'

...
class SectorPhaseEmbedding(tf.keras.layers.Layer):
    def __init__(self, num_sectors, num_phases, embed_dim, **kwargs):
        super(SectorPhaseEmbedding, self).__init__(**kwargs)
        self.num_sectors = num_sectors
        self.num_phases = num_phases
        self.sector_emb = tf.keras.layers.Embedding(input_dim=num_sectors, output_dim=embed_dim)
        self.phase_emb = tf.keras.layers.Embedding(input_dim=num_phases, output_dim=embed_dim)

    def call(self, x):
        # x is shape (batch, seq_len, embed_dim)
        seq_len = tf.shape(x)[-2]
        
        positions = tf.range(start=0, limit=seq_len, delta=1)
        
        # For a 1250 sequence:
        # sector_ids: 0..0, 1..1, ..., 4..4
        # phase_ids: 0..249, 0..249, ..., 0..249
        sector_ids = positions // self.num_phases
        phase_ids = positions % self.num_phases
        
        sector_embs = self.sector_emb(sector_ids)
        phase_embs = self.phase_emb(phase_ids)
        
        return x + sector_embs + phase_embs
...
def build_exp10c_model():
    """
    Exp 10C Architecture (Sector-Aware CLS Transformer)
    """
    ...
    # 3. Sector-Phase 2D Positional Encoding
    embedded_sequence = SectorPhaseEmbedding(num_sectors=5, num_phases=250, embed_dim=64)(unrolled_sequence)
...
def main():
    print(f"\n==================================================")
    print(f"TRAINING EXP 10C: Sector-Aware CLS Transformer")
    print(f"==================================================")
    ...
    save_path = 'data/models/exp10c_model.keras'

...
def main():
    ...
    plt.title('Ternary Confusion Matrix (Baseline)')
    plt.ylabel('Actual True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    os.makedirs(os.path.join('docs', 'assets'), exist_ok=True)
    plt.savefig(os.path.join('docs', 'assets', 'v2_baseline_cm.png'))
    plt.close()
    
    print("Generating Report...")
    markdown_content = f"""# V2_BASELINE_001
...
"""
    with open('docs/V2_BASELINE_001.md', 'w') as f:
        f.write(markdown_content)
        
    print("Baseline evaluation complete. Saved to docs/V2_BASELINE_001.md")

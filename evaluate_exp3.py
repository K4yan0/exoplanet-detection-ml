...
def main():
    print("Loading Multi-Sector data...")
    dataset_path = os.path.join('data', 'tess_ml_arrays', 'tess_dataset_exp3.npz')
    ...
    plt.title('Ternary Confusion Matrix (Exp 3 - Multi-Sector)')
    plt.ylabel('Actual True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    os.makedirs(os.path.join('docs', 'assets'), exist_ok=True)
    plt.savefig(os.path.join('docs', 'assets', 'v2_exp3_cm.png'))
    plt.close()
    
    print("Generating Report...")
    markdown_content = f"""# V2_EXP3_001

## Configuration
* **Model**: `exoplanet_cnn_v1.keras` (Frozen V1 Reference)
* **Dataset**: `tess_dataset_exp3.npz` (Test Split: 20%, Seed 42, Stratified)
* **Preprocessing Contract**: TERNARY_V1
  * **Sectors: Up to 5 (Stitched)**
  * Outlier Removal: OFF
  * SG Window: 101
...
## Artifacts
![Confusion Matrix](assets/v2_exp3_cm.png)
...
"""
    with open('docs/V2_EXP3_001.md', 'w') as f:
        f.write(markdown_content)
        
    print("Exp3 evaluation complete. Saved to docs/V2_EXP3_001.md")
